...
class ImageViewer:
    def __init__(
        ...
        self.key_bindings: KeyBindingsType = key_bindings
    ...
    def display_image(self, index: int) -> None:
        ...
        tk_image: ImageTk.PhotoImage = ImageTk.PhotoImage(pil_image)
        self.label.configure(image=tk_image)
        self.current_image = tk_image  # Keep a reference to prevent garbage collection

        self.root.title(f"QC ({index + 1}/{len(self.image_paths)})")
